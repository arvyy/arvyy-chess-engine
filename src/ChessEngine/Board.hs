...
clearPosition (ChessBoardPositions black white bishops horses queens kings pawns rocks) x y =
    ...
    where bit = coordsToBitIndex x y
...
pieceOnSquare' (ChessBoardPositions black white bishops horses queens kings pawns rocks) x y = do
    ...
    where bit = coordsToBitIndex x y
          color = if testBit black bit
                  then Just Black
                  else if testBit white bit
                  then Just White
                  else Nothing
          pieceType = if testBit bishops bit
                      then Just Bishop
                      else if testBit horses bit
                      then Just Horse
                      else if testBit queens bit
                      then Just Queen
                      else if testBit kings bit
                      then Just King
                      else if testBit pawns bit
                      then Just Pawn
                      else if testBit rocks bit
                      then Just Rock
                      else Nothing
...
applyMoveUnsafe board (Move x y x' y' promotion) = 
    let ChessPiece player pieceType = case pieceOnSquare board x y of
        ...
        newPieces = if isEnPassantMove
                    then applyEnPassant oldPieces player
                    else if isKingCastleMove 
                    then applyKingCastle oldPieces player
                    else if isQueenCastleMove
                    then applyQueenCastle oldPieces player
                    else applyNormalMove oldPieces player pieceType
        whiteKingCastle' = (not (isCastleMove && player == White)) && 
                           (whiteKingCastle board) && 
                            case (player, pieceType) of
                                (White, King) -> False
                                (White, Rock) -> if (x == 1 && y == 8) then False else True
                                _ -> True
        whiteQueenCastle' = (not (isCastleMove && player == White)) &&
                            (whiteQueenCastle board) && 
                            case (player, pieceType) of
                                (White, King) -> False
                                (White, Rock) -> if (x == 1 && y == 1) then False else True
                                _ -> True
        blackKingCastle' = (not (isCastleMove && player == Black)) &&
                           (blackKingCastle board) && 
                           case (player, pieceType) of
                                (Black, King) -> False
                                (Black, Rock) -> if (x == 1 && y == 8) then False else True
                                _ -> True
        blackQueenCastle' = (not (isCastleMove && player == Black)) &&
                            (blackQueenCastle board) && 
                            case (player, pieceType) of
                                (Black, King) -> False
                                (Black, Rock) -> if (x == 1 && y == 1) then False else True
                                _ -> True
    in board 
        { pieces = newPieces
...
pieceThreats board (x, y, ChessPiece color Horse) = 
    let candidates = [(x + 1, y + 2), (x + 1, y - 2), (x - 1, y + 2), (x - 1, y - 2),
...
pieceThreatsRay color board (x, y) (dx, dy) =
    let squares = (filter (\ square -> inBounds (fst square) (snd square))
                   . map (\ i -> (x + i * dx, y + i * dy)))
                  [1..7]
...
squareUnderThreat board player x y =
    let opponentPieces = filter (\ p -> case p of 
...
pawnCandidateMoves board x y player =
    let (dir, inStartingPos, inEnPassantPos, promotesOnMove) = if player == White
                                                               then (1, y == 2, y == 5, y == 7)
                                                               else (-1, y == 7, y == 4, y == 2)
        y' = y + dir
        aheadIsClear = inBounds x (y + 2*dir) && case pieceOnSquare board x y' of
                        Just _ -> False
                        Nothing -> True
        normalCaptures = do
            x' <- [x - 1, x + 1]
        ...
        enPassantCaptures = do
            ...
            x' <- case (enPassant board) of
                    ...
                    _ -> []
            return (Move x y x' y' Nothing)
        doubleDipMove = let doubleAheadIsClear = inBounds x (y + 2*dir) && case pieceOnSquare board x (y + 2*dir) of
                                                    Just _ -> False
                                                    Nothing -> True
                            canDoubleDip = inStartingPos && aheadIsClear && doubleAheadIsClear
                        in if canDoubleDip then [Move x y x (y + 2 * dir) Nothing] else []
        singleMove = if aheadIsClear && not promotesOnMove 
                     then [Move x y x (y + dir) Nothing] 
                     else if aheadIsClear && promotesOnMove
...
canCastleKingSide board color =
    let hasRights = if color == White then (whiteKingCastle board) else (blackKingCastle board)
        ...
        hasEmptySpaces = case (pieceOnSquare board 6 y, pieceOnSquare board 7 y) of
                            ...
                            _ -> False
        travelsThroughCheck = case filter (\x' -> not (squareUnderThreat board color x' y)) [5, 6, 7] of
                                [] -> False
                                _ -> True
...
canCastleQueenSide board color =
    let hasRights = if color == White then (whiteQueenCastle board) else (blackQueenCastle board)
        ...
        hasEmptySpaces = case (pieceOnSquare board 2 y, pieceOnSquare board 3 y, pieceOnSquare board 4 y) of
                            ...
                            _ -> False
        travelsThroughCheck = case filter (\x' -> not (squareUnderThreat board color x' y)) [3, 4, 5] of
                                [] -> False
                                _ -> True
    in hasRights && hasEmptySpaces && (not travelsThroughCheck)
...
kingCandidateMoves board x y player =
    let baseMoves = map (\pos -> Move x y (fst pos) (snd pos) Nothing)
...
candidateMoves' board =
    let player = (turn board)
        playerPieces = filter (\ p -> case p of 
